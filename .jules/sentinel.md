...
## 2025-05-20 - Command Wrapper Bypass via Start-of-String Regex and ReadOnly Allowlist (Regression/Gap)

**Vulnerability:** The security interceptor continued to use start-of-string or operator boundaries `(?:^|[|&;])` for forbidden commands like `sudo`, `su`, `useradd`, etc. This allowed bypasses when forbidden commands were passed as arguments to command wrappers like `time`, `nohup`, `stdbuf`, `timeout`, `nice`, `ionice`, `taskset`, `watch`, etc. (e.g., `time sudo rm -rf /`).
**Learning:** Command line argument parsing and classification must account for all possible command runners and wrappers which execute their arguments. If the command wrapper isn't unwrapped, the forbidden checks fail.
**Prevention:** Use a space character `\s` inside the non-capturing group for boundaries `(?:^|[|&;\s])` to catch forbidden binaries anywhere in the command string, or correctly unwrap ALL possible wrapper binaries. We chose to update the regex boundary.
